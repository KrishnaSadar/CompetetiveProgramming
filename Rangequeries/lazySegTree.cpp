...
class SegmentTree{
    public:
      ...
      void build(int st, int ed, int idx){
        ...
        a[idx] = combine(a[2*idx], a[2*idx+1]);
      }

      void push(int st, int ed, int idx){
        if(lazy[idx] != 0){
            a[idx] += (ed - st + 1) * lazy[idx];
            if(st != ed){
                lazy[2*idx] += lazy[idx];
      ...
      void rec(int st, int ed, int idx, int l, int r, ll val){
        push(st, ed, idx);
        if(ed < l || r < st) return;
        if(st >= l && ed <= r){
            lazy[idx] += val;
        ...
        int mid = (st + ed) / 2;
        rec(st, mid, 2*idx, l, r, val);
        rec(mid+1, ed, 2*idx+1, l, r, val);
        a[idx] = combine(a[2*idx], a[2*idx+1]);
      }
      ...
      ll rec1(int st, int ed, int idx, int l, int r){
        ...
        int mid = (st + ed) / 2;
        ll left = rec1(st, mid, 2*idx, l, r);
        ll right = rec1(mid+1, ed, 2*idx+1, l, r);
