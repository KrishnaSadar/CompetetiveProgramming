...
void forward1(vector<ll>&dp){
    // subSet contribution to superset
  for(int b=0;b<=LOG;b++) for(int i=0;i<=sz;i++) if(i&(1<<b)) dp[i]+=dp[i^(1<<b)];
}
void backward1(vll &dp){
    // undo of forward 1
 for(int b=LOG;b>=0;b--) for(int i=sz;i>=0;i--) if(i&(1<<b)) dp[i]-=dp[i^(1<<b)];
}
void forward2(vll &dp){
    // superset contributes to subset
    for(int b=0;b<=LOG;b++) for(int i=0;i<=sz;i++) if(i&(1<<b)) dp[i^(1<<b)]+=dp[i];
}
void backward2(vll &dp){
    // undo of forward 2
    for(int b=LOG;b>=0;b--)for(int i=sz;i>=0;i--)if(i&(1<<b))dp[i^(1<<b)]+=dp[i];
}
void solve(){
    ll n;cin>>n;
    vll dp1(sz+5,0),dp2(sz+5,0);
    vll arr(n);
    for(int i=0;i<n;i++){
        ll x;cin>>x;
        arr[i]=x;
